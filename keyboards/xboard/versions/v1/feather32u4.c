#include "feather32u4.h"
#include "../../gpio/mcp23017.h"
#include "debug.h"

uint8_t reverse_byte(uint8_t x)
{
    static const uint8_t table[] = {
        ,
    };
    return table[x];
}

void select_row(uint8_t row) {
    row = 0xFF & ~(1<<row);
    mcp23017_write(MCP23017_ADDR, MCP23017_REG_GPIOA, row);
}

void unselect_row(uint8_t row) {
     mcp23017_write(MCP23017_ADDR, MCP23017_REG_GPIOA, 0xFF);
}

uint32_t read_row(void) {
    uint32_t banks = 0;
    
    uint8_t mcp0b_gpio = 0;
    uint8_t mcp1a_gpio = 0;
    uint8_t mcp1b_gpio = 0;
    mcp23017_read(MCP23017_ADDR, MCP23017_REG_GPIOB, &mcp0b_gpio);
    mcp23017_read(MCP23017_ADDR + 0xE, MCP23017_REG_GPIOA, &mcp1a_gpio);
    mcp23017_read(MCP23017_ADDR + 0xE, MCP23017_REG_GPIOB, &mcp1b_gpio);

    banks |= ((uint32_t)reverse_byte(mcp1b_gpio) << 16);
    banks |= ((uint16_t)reverse_byte(mcp1a_gpio) << 8);
    banks |= reverse_byte(mcp0b_gpio);

    return banks;
}

void matrix_read_cols_on_row(matrix_row_t current_matrix[], uint8_t current_row) {
    matrix_row_t current_row_value = 0;

    select_row(current_row);
    matrix_output_select_delay();

    current_row_value = read_row();

    // // Unselect row
    unselect_row(current_row);
    matrix_output_unselect_delay(current_row, current_row_value != 0); // wait for all Col signals to go HIGH

    // Update the matrix
    current_matrix[current_row] = current_row_value;
}

void keyboard_post_init_user(void) {
    debug_enable=true;
    debug_matrix=true;
    debug_keyboard=true;

    mcp23017_init(MCP23017_ADDR, MCP23017_BANK_OUTPUT, MCP23017_BANK_INPUT);
    mcp23017_init(MCP23017_ADDR + 0xE, MCP23017_BANK_INPUT, MCP23017_BANK_INPUT);
}

void matrix_scan_user(void) {
    //matrix_print();
}